...
import {createMagazineFB} from '../modules/redux/magazineItems';
...
const Write = ({nickname, userID, type}) => {
	...
	console.log(titleText,descText,file.current?.files.length)
	const getDate = () => {
		...
		const result = `${year}-${month}-${date}, ${dayArr[day]}, ${_date.getHours()}:${_date.getMinutes()}`;
		return result;
	}
	useEffect(()=>{
		setContent((prev)=>{
	...
	const [validation, setValidation] = useState([false, false, false]); // ref들 빈칸 여부 체크할 state
	useEffect(()=>{
		setValidation(()=>{
			const _inputs = inputs.map((v,i)=>{
				console.log(v?.value)
				if(v?.value === undefined || v?.value === 0) return false;
				else return true;
			})
			console.log(_inputs)
			return _inputs;
		})
	},[titleText,descText,imgFile]);
	console.log(validation)
	const registerMagazine = async () => { // 등록하기 클릭했을 경우 발생하는 이벤트
		await inputs.reduce((acc,cur,idx)=>{
			if(cur?.value.trim() === '') { // 빈칸이거나 공백만 있을 경우 
				setValidation((props)=>{
					const _props = [...props];
					_props[idx] = false;
					return [..._props];
	...
	// useEffect(()=>{ // validation 값 바뀔때마다 업데이트(등록하기 버튼 누를 때)
	...
	// },[validation]);
	// const checkEmpty = () => {
	// 	for(let i=0; i<validation.length; i++){ // false 있을 경우(빈칸) focus
	// 		if(validation[i] === false){
	// 			inputs[i].focus();
	// 			break;
	// 		}
	// 		if(i+1 === validation.length){ // 빈칸 없을 경우
	// 			console.log('dispatch')
	// 			dispatch(createMagazineFB(content));
	// 		}
	// 	}
	// };
		...
		<div className="contents">
					...
					<div>
						<input
							...
							onInput={() => {
								...
								setTitleText(title.current.value)
							}}
						/>
						{!validation[0] && <p className="txt_error">장소 이름은 필수 항목입니다.</p>}
					</div>
				</div>
			</InputArea>
			<InputArea className="file">
				...
				<div className="input_box">
					...
					{!validation[1] && <p className="txt_error">이미지는 필수 항목입니다.</p>}
					{!noImage.current && <p className="txt_error">이미지 업로드가 취소되었습니다. 파일을 다시 첨부해주세요.</p>}
				</div>
			</InputArea>
			<InputArea className="textarea">
				...
				<div className="input_box">
					...
					{!validation[2] && <p className="txt_error">설명은 필수 항목입니다.</p>}
				</div>
			</InputArea>
			...
			<section>
				<PreviewArea>
					...
					<MagazineItem page="write" layout={content.layout} content={content}></MagazineItem>
				</PreviewArea>
			</section>
			<ButtonArea>
